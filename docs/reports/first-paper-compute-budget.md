...
SEN2SRLite CPU 为 0.209878 秒／次，bicubic CPU 为 0.006185 秒／次。
48 ROI 的模型推理部分外推约 6.637 分钟，不含解包、完整评分、缓存写盘等开销，
也不等于整机计费时长。SEN2SRLite 新旧输出哈希差异仍待核查；不能宣称全部模型
逐字节复现或完整外部预算已完成。服务器已通知可关机并保留数据盘。
