...
新隔离入口已实现实际 SR→HR 协调配准失败检测、有限支持分母和缺失处理，
详见 [本地集成检查点](first-paper-external-local-integration.md)。
使用全512网格合成样本调用真实上游成功，两轮复算一致；这不证明 Spain 有效率。
参数和失败语义见 `docs/superpowers/specs/first-paper-opensr-failure-design.md`。
旧七标量适配器保持原样；最终外部协议仍未冻结。
