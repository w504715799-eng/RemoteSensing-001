...
本阶段把 LDSR-S2 作为第三个黑盒 RGBN ×4 超分辨率基线接入 TrustSR，并在满足能力门槛的租用单张 GPU 上完成可复现推理。实施继续遵守“小步通过、失败即停”的原则：先验证环境，再运行单样本，再运行固定 9 样本；任何一步失败都不扩大规模。
...
- 记录单图耗时、峰值显存和远程运行环境，但把这些非确定字段放入独立运行报告，不写入确定性指标 JSON；耗时只在记录的同一硬件内比较，哈希、重复性和质量指标仍是可比较的科学关卡；
...
2. 强制推理一次，记录输出摘要、耗时和峰值显存；耗时只与相同已记录硬件上的运行比较；
3. 恢复相同初始条件再次强制推理；
...
- SSH 主机指纹变化且用户未确认；
- CUDA 不可用、可见 GPU 数不等于一、compute capability 不是数值 `major.minor` 或低于 `8.0`、可用显存不足 18 GiB，或存在未知 GPU 进程；
