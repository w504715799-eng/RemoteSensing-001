"""Executable contracts for the Phase 2B2-A CPU cloud runner."""
...
RUNNER = REPOSITORY / "scripts" / "phase2b2a" / "run_cloud.sh"
...
def _environment(
    ...
    *,
    ...
    _make_executable(
        ...
        "    printf '/dev/fake 4000000 1 %s 1%% /persistent\\n' \"$FAKE_AVAILABLE_KIB\"\n"
        "    ;;\n"
        "  -Pi)\n"
    ...
    for name in ("conda", "pip", "nvidia-smi", "wget", "curl"):
        _make_executable(
            fake_bin / name,
            "#!/usr/bin/env bash\n"
...
def _fake_base_python(tmp_path: Path) -> tuple[Path, dict[str, str], Path]:
    ...
    _make_executable(
        ...
        "if arguments[:3] == ['-m', 'trustsr.cli.phase2b2a', 'audit-inputs']:\n"
        "    print(json.dumps({'stage': 'audit-inputs'}, separators=(',', ':')))\n"
        "    raise SystemExit(0)\n"
...
def test_runner_uses_base_python_without_prohibited_commands(tmp_path: Path) -> None:
    storage_root = tmp_path / "persistent"
    completed, calls, prohibited = _invoke(tmp_path, storage_root=storage_root)

    assert completed.returncode == 0, completed.stderr
    assert calls == [
        [
            "-m",
            "trustsr.cli.phase2b2a",
            "audit-inputs",
            "--storage-root",
            str(storage_root),
    ...
    assert json.loads(completed.stdout) == {"stage": "audit-inputs"}
    log = storage_root / "trustsr" / "phase2b2a" / "logs" / "audit-inputs.jsonl"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"available_kib": 1024 * 1024}, "more than 1 GiB"),
