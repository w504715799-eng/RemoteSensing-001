...
require_stage() {
  case "$1" in
    preflight | single | smoke | replay) printf '%s\n' "$1" ;;
    *) die 'stage must be preflight, single, smoke, or replay' ;;
  esac
}
...
run_main() {
  ...
  local selection_manifest=false
  local selection_digest=false
  local input_audit=false
  local input_digest=false
  local sen2srlite_dir=false
  local ldsr_dir=false
  local available_kib
  ...
  stage="$(require_stage "$3")"
  shift 3
  for argument in "$@"; do
    case "$argument" in
      --confirm-cloud-storage) confirmed=true ;;
      --selection-manifest) selection_manifest=true ;;
      --selection-manifest-sha256) selection_digest=true ;;
      --input-audit) input_audit=true ;;
      --input-audit-sha256) input_digest=true ;;
      --sen2srlite-model-dir) sen2srlite_dir=true ;;
      --ldsr-model-dir) ldsr_dir=true ;;
      --st*) die 'stage arguments must not override storage root' ;;
    esac
  done
  [[ "$confirmed" == true ]] ||
    die 'stage arguments must include --confirm-cloud-storage'
  [[ "$selection_manifest" == true && "$selection_digest" == true ]] ||
    die 'stage arguments must include the frozen selection manifest and SHA-256'
  [[ "$input_audit" == true && "$input_digest" == true ]] ||
    die 'stage arguments must include the frozen input audit and SHA-256'
  [[ "$sen2srlite_dir" == true && "$ldsr_dir" == true ]] ||
    die 'stage arguments must include both model directories'
  ...
  (( available_kib > 8 * 1024 * 1024 )) ||
    die 'more than 8 GiB of free disk space is required'
  ...
  log_dir="$storage_root/trustsr/phase2b2b/logs"
