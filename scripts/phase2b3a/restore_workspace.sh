...
run_main() {
  ...
  local sen2srlite_source ldsr_source checkpoint_directory
  ...
  [[ "$verify_commit" == "$git_head" ]] || die 'checkpoint reviewed commit does not match checkout HEAD'
  rm -f -- "$verify_file" || die 'checkpoint verification output cannot be removed'
  verify_file=

  reject_symlink_components "$model_mount_directory" || die 'model mount directory contains a symlink'
  ...
  local restore_record="$record_output"
