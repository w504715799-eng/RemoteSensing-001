...
Spain 外部评估未执行；候选使用固定 HF 提交下的 v2／100 包与同版本 CSV，
避免跨版本成员映射假设。包内成员／行顺序仍需在冻结后的受控加载中核验。
版本、来源和独立性限制见元数据审计。
...
LDSR 与 bicubic 输出哈希匹配历史记录；SEN2SRLite 的 3 个输出哈希均不同，虽输入、
权重和已记录身份一致，差异原因及幅度仍未确定，最终外部协议冻结前必须核查。
