"""Fixed internal_test-only LDSR K5 score and R9 risk maps.

This module deliberately owns no conformal parameter or threshold fitting.  It
accepts only the frozen internal_test prediction bundle and exposes one score and
...
def _validate_fixed_score_identity(identity: ScoreIdentity) -> None:
    """Reject a directly-constructed score identity outside the B3-C contract."""
    ...
    required = {
        ...
        "phase2b3b_producer_revision": PRODUCER_REVISION,
    }
    if (
        ...
        raise ValueError("cached internal_test score identity is outside the fixed B3-C contract")


@dataclass(frozen=True)
class CachedInternalTestScore:
    ...
    def __post_init__(self) -> None:
        if self.name != SCORE_NAME:
            raise ValueError("cached internal_test score has the wrong score name")
        if not isinstance(self.identity, ScoreIdentity):
            raise TypeError("cached internal_test score requires a ScoreIdentity")
        if (
            self.identity.score_name != SCORE_NAME
            or self.identity.score_schema_version != SCORE_SCHEMA_VERSION
        ):
            raise ValueError("cached internal_test score identity is invalid")
        _validate_fixed_score_identity(self.identity)
        _validate_map_tensor(self.tensor, label="cached internal_test score", upper_bound=0.25)
        if not _is_sha256(self.score_sha256) or self.score_sha256 != tensor_sha256(self.tensor):
            raise ValueError("cached internal_test score digest is invalid")


@dataclass(frozen=True)
class InternalTestMaps:
    """The sole permitted score/risk maps for one internal_test ROI."""

    sample_id: str
    score: CachedInternalTestScore
    ...
    def __post_init__(self) -> None:
        if type(self.sample_id) is not str or not self.sample_id:
            raise TypeError("internal_test maps sample_id must be a non-empty string")
        if not isinstance(self.score, CachedInternalTestScore):
            raise TypeError("internal_test maps requires a CachedInternalTestScore")
        self.score.__post_init__()
        if self.score.identity.sample_id != self.sample_id:
            raise ValueError("internal_test score identity does not match the sample")
        if (
            type(self.score_prediction_sha256s) is not tuple
            or len(self.score_prediction_sha256s) != len(SEEDS)
        ):
            raise ValueError("internal_test maps requires the ordered fixed K5 prediction digests")
        if not all(_is_sha256(digest) for digest in self.score_prediction_sha256s):
            raise ValueError("internal_test prediction digests are invalid")
        if self.score.identity.input_sha256s != self.score_prediction_sha256s:
            raise ValueError("internal_test score identity prediction digests are invalid")
        if self.risk_name != RISK_NAME or self.risk_window != RISK_WINDOW:
            raise ValueError("internal_test maps has the wrong fixed risk configuration")
        _validate_map_tensor(self.risk, label="internal_test risk", upper_bound=1.0)
        if tuple(self.risk.shape) != tuple(self.score.tensor.shape):
            raise ValueError("internal_test score and risk shapes differ")
        if not _is_sha256(self.risk_sha256) or self.risk_sha256 != tensor_sha256(self.risk):
            raise ValueError("internal_test risk digest is invalid")


def _validate_pair(pair: LoadedCrosssensorPair) -> LoadedCrosssensorPair:
    if not isinstance(pair, LoadedCrosssensorPair):
        raise TypeError("internal_test maps require a LoadedCrosssensorPair")
    pair.pair.validate()
    metadata: CrosssensorPairMetadata = pair.metadata
    if type(metadata.split) is not str or metadata.split != "internal_test":
        raise ValueError("internal_test maps require the internal_test split")
    ...
    if (
        ...
        raise ValueError("internal_test pair has the wrong crop or normalization policy")
...
def _validate_bundle(
    ...
    if not isinstance(bundle, InternalTestPredictionBundle):
        raise TypeError("internal_test maps require a InternalTestPredictionBundle")
    if type(bundle.sample_id) is not str or bundle.sample_id != pair.pair.sample_id:
        raise ValueError("internal_test prediction bundle sample does not match the pair")
    if type(bundle.items) is not tuple or tuple(item.seed for item in bundle.items) != SEEDS:
        raise ValueError("internal_test prediction bundle must contain the ordered fixed K5 seeds")
    for seed, item in zip(SEEDS, bundle.items, strict=True):
        if not isinstance(item, CachedInternalTestPrediction):
            raise TypeError("internal_test prediction bundle item is invalid")
        ...
        if (
            ...
            raise ValueError("internal_test prediction bundle input identity differs from the pair")
    return bundle
...
def _load_or_compute_score(
    identity: ScoreIdentity, bundle: InternalTestPredictionBundle, score_cache: ScoreCache
    ...
    if score is None:
        ...
        if score is None or tensor_sha256(score) != produced_sha256:
            raise RuntimeError("internal_test score differs after cache commit")
    return CachedInternalTestScore(SCORE_NAME, identity, tensor_sha256(score), score)


def load_or_compute_internal_test_maps(
    pair: LoadedCrosssensorPair,
    bundle: InternalTestPredictionBundle,
    score_cache: ScoreCache,
) -> InternalTestMaps:
    """Load or compute the only permitted internal_test K5 score and R9 risk.

    The score is cache-backed.  The risk is deliberately computed in memory for
    this invocation and is neither cached nor serialized as a internal_test unit.
    """
    ...
    if not isinstance(score_cache, ScoreCache):
        raise TypeError("internal_test maps require a ScoreCache")
